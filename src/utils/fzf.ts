...
export class FzfSelector {
  ...
  constructor(processRunner?: ProcessRunner) {
    this.processRunner = processRunner ?? new BunProcessRunner();
  }
  ...
  async isFzfAvailable(): Promise<boolean> {
    try {
      const result = await this.processRunner.run(['fzf', '--version'], {
        stdout: 'pipe',
        stderr: 'pipe'
      });
  ...
  private async runFzf(
    ...
    try {
      const result = await this.processRunner.run(['bash', '-c', shellCmd], {
        stdin: 'inherit',
        stdout: 'pipe',
        stderr: 'inherit',
        env: {
