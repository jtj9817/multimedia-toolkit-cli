...
| File | Action | Status |
| :--- | :--- | :--- |
| `docs/plans/2026-08-29-output-directory-selection.md` | Create | 🟢 |
...
| `src/cli/commands/interactive-commands.ts` | Modify | 🟢 |
| `src/utils/path.ts` | Modify | 🟢 |
| `tests/app/paths.test.ts` | Create | 🟢 |
| `tests/cli/output-destination.test.ts` | Create | 🟢 |
| `tests/utils/fzf.test.ts` | Modify (extended) | 🟢 |
| `tests/config/config.test.ts` | Modify (extended) | 🟢 |
| `tests/utils/path.test.ts` | Modify (extended) | 🟢 |
| `tests/utils/logger.test.ts` | Modify (log location) | 🟢 |
| `test-fzf.ts` | Modify | 🟢 |
| `README.md` | Modify | 🟢 |
| `docs/configuration.md` | Modify | 🟢 |

**Note**: Tests were placed under `tests/` (the repository's dominant convention — 15 pre-existing suites) and existing files extended where they already covered the subject, instead of creating the originally planned `src/**` test files.
...
Executed 2026-08-29. All four phases implemented and verified.

### Verification Results

| Category | Metric |
| :--- | :--- |
| `bun test` | 118 pass, 0 fail (314 expect calls, 19 files; 94 before the change) |
| `bun build src/index.ts --outdir dist --target bun` | Succeeds (205 KB bundle) |
| `bun run typecheck` (tsc strict) | Clean |
| `bun run lint` (oxlint) | 0 errors (pre-existing warnings only) |
| `bun run test-fzf.ts` | Requires an interactive TTY — directory-mode smoke added as Test 4; needs a manual run |

### New Files

| File | Purpose |
| :--- | :--- |
| `tests/app/paths.test.ts` | cwd fallback, env-var and explicit-override precedence |
| `tests/cli/output-destination.test.ts` | Dialog FSM: default, browse→confirm, canceled browse, manual entry, empty-input loop, back-out, mkdir failure recovery, no-fzf degradation, shared single-output flow |

### Modified Files

| File | Changes |
| :--- | :--- |
| `src/app/paths.ts` | `cwd` option; default resolution now explicit → env var → `process.cwd()` |
| `src/config/config.ts` | Legacy `~/Music/AudioExtracted` migration on load (file + DB paths); `defaultOutputDir` persisted only as an explicit override; `resetOutputDir()` |
| `src/utils/logger.ts` | Optional `logDir` constructor dep (old behavior as fallback) |
| `src/app/context.ts` | Logs wired to `paths.baseDir/logs` |
| `src/utils/fzf.ts` | `entryType: 'files' \| 'directories'` through `FzfOptions`/`buildFzfShellCommand`/`selectFiles`; `FzfSelector.selectDirectory` (directories only, `maxDepth 5`, no preview) |
| `src/cli/interface.ts` | Public `isFzfAvailable()` (two call sites refactored); `selectDirectoryWithFzf` feedback loop with confirm/reselect/manual |
| `src/cli/dialogs/output-destination.ts` | Shared `chooseOutputDirectory` (default/browse/manual numbered menu) used by both dialog entry points; `prepareDirectory` creates the directory and reports failures; `~` expansion; both prompts now return `null` on user back-out |
| `src/cli/menus/settings-menu.ts` | Output-directory change via the fzf picker; new "Reset output directory to startup default" entry |
| `src/cli/commands/interactive-commands.ts` | Null guards at all eight dialog call sites (download-clip flow skips clipping but still logs the successful download) |
| `src/utils/path.ts` | `expandHomePath` shared by the interface and dialog |
| `test-fzf.ts` | Directory-selection smoke test (Test 4) |
| `README.md`, `docs/configuration.md` | New default, env var, override semantics, settings menu renumbering |
| `tests/utils/fzf.test.ts`, `tests/config/config.test.ts`, `tests/utils/path.test.ts`, `tests/utils/logger.test.ts` | Extended for directory mode, override semantics, `expandHomePath`, new log location |

### Deviations from Plan

1. Tests live under `tests/` (existing convention) rather than the planned `src/**` paths; existing suites were extended where present.
2. Task 3.2's `this.cli['fzf']` sketch was replaced by a public `CLIInterface.isFzfAvailable()` method, which the two existing selection loops now share.
3. `~` expansion implemented once as `expandHomePath` in `src/utils/path.ts` instead of separately in the dialog and interface.
